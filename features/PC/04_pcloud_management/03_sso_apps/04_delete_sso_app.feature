Feature: Pcloud Management - Delete SSO APP
  ...
  Scenario: [04_03_04]
    ...
    Given the super admin has already signed in
     ...
     Then the super admin should be at page - "/admin/pcloud/sso_apps/1"
      And should see "刪除"
     When the super admin clicks "刪除"
     When the super admin accepts a confirm message - "您確定要執行？"
     Then the super admin should be at page - "/admin/pcloud/sso_apps"
      And should see "SSO APP 列表"
      And should see "SSO_APP1 已刪除"
      And should not see "名稱 SSO_APP1"
