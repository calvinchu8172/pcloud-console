Before '@proxy' do
  ...
  # OAuth - Authorize 頁面
  proxy.stub("#{ENV['ZYXEL_OAUTH_APP_URL']}/oauth/authorize").and_return(
    Proc.new { |params, headers, body|
      ...
      if @current_resource_owner
        ...
        @stored_location = "#{ENV['ZYXEL_OAUTH_APP_URL']}/oauth/authorize?#{params.to_query}"
        { redirect_to: "#{ENV['ZYXEL_OAUTH_APP_URL']}/users/sign_in" }
      end
    }
  )
  # OAuth - 送出 Authorize 頁面表單
  proxy.stub("#{ENV['ZYXEL_OAUTH_APP_URL']}/oauth/authorize", method: 'post').and_return(
    Proc.new { |params, headers, body|
      # 從 body 取得 params
      ...
      if commit == 'Authorize'
        ...
        if client_id == ENV['ZYXEL_OAUTH_APP_ID'] && response_type == 'code'
  ...
  # OAuth - Sign In 頁面
  proxy.stub("#{ENV['ZYXEL_OAUTH_APP_URL']}/users/sign_in").and_return(
    Proc.new { |params, headers, body|
  ...
  proxy.stub("#{ENV['ZYXEL_OAUTH_APP_URL']}/users/sign_in", method: 'post').and_return(
    Proc.new { |params, headers, body|
      # 從 body 取得 params
      # binding.pry
      params = Rack::Utils.parse_nested_query(body).deep_symbolize_keys
  ...
  # OAuth - 登出頁面
  proxy.stub("#{ENV['ZYXEL_OAUTH_APP_URL']}/oauth/logout").and_return(
    Proc.new { |params, headers, body|
      # 取得 params
      params = params.inject({}) do |h, (k,v)|
        h[k] = v[0]
